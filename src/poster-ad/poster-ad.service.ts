...
@Injectable()
export class PosterAdService {
  constructor(
    @InjectRepository(PosterAd) private posterAdRepo: Repository<PosterAd>,
  ...
  async createAd(
    ...
    const positionValue =
      side === PositionType.CENTER_TOP || side === PositionType.CENTER_BOTTOM
        ? 0 // Using 0 as a placeholder for null
        : position;
    ...
    const advert = this.posterAdRepo.create(ad);
    ...
    return await this.posterAdRepo.save(advert);
  }

  async findOne(id: string): Promise<PosterAd> {
    const ad = await this.posterAdRepo.findOne({
      where: { id },
      relations: [
        'customer',
        'customer.user',
  ...
  async updateAdByUser(
    ...
    return this.posterAdRepo.save(ad);
  }

  async updateAdByAdmin(
    userId: string,
    adId: string,
    updateDto: Partial<CreatePosterAdDto>,
  ): Promise<PosterAd> {
    ...
    if (ad?.position && position !== undefined) {
      console.log(position);

      const positionValue =
        position.side === PositionType.CENTER_TOP ||
        position.side === PositionType.CENTER_BOTTOM
          ? 0 // Using 0 as a placeholder for null
          : position.position;

      console.log('pv', positionValue);
      await this.adPositionService.update(ad.position.id, {
  ...
  async updateAdStatus(adId: string, status: AdStatus): Promise<PosterAd> {
    const ad = await this.posterAdRepo.findOne({
      where: { id: adId },
      relations: ['position'],
    });
  ...
  async deleteAdByAdmin(adId: string, userId: string): Promise<PosterAd> {
    const ad = await this.posterAdRepo.findOne({
      where: { id: adId },
      relations: ['position'],
    });
  ...
  async findAllByStatuses(statuses: AdStatus[]): Promise<PosterAd[]> {
    const ads = await this.posterAdRepo.find({
  ...
  async findAllByUserId(userId: string): Promise<PosterAd[]> {
    const user = await this.userService.findOneById(userId);
    return await this.posterAdRepo.find({
  ...
  async findByCategory(categoryId: string): Promise<PosterAd[]> {
    return this.posterAdRepo
      .createQueryBuilder('ad')
      .leftJoinAndSelect('ad.mainCategory', 'mainCategory')
      .leftJoinAndSelect('ad.categoryOne', 'categoryOne')
      .leftJoinAndSelect('ad.categoryTwo', 'categoryTwo')
  ...
  async getPosterAdStats() {
    const total = await this.posterAdRepo.count({ where: { isActive: true } });
    const statusCounts = {} as Record<string, number>;
    for (const status of Object.values(AdStatus)) {
      statusCounts[status] = await this.posterAdRepo.count({
        where: { status, isActive: true },
      });
    }
    const cityRows = await this.posterAdRepo
      .createQueryBuilder('ad')
      .select('ad.city', 'city')
      ...
      .groupBy('ad.city')
      .getRawMany();
    const stateRows = await this.posterAdRepo
      .createQueryBuilder('ad')
      ...
      .groupBy('ad.state')
      .getRawMany();
    const categoryRows = await this.posterAdRepo
      .createQueryBuilder('ad')
